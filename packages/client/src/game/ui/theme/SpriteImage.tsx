...
export const SpriteImage = ({
  ...
  if (!spriteAtlasEntry)
    throw new Error(`Could not find sprite ${spriteKey} in atlas`);
  ...
  const imgStyle = {
    ...
    clipPath: `inset(${spriteAtlasEntry.frame.y + 1}px ${
      atlasDimensions.w -
      (spriteAtlasEntry.frame.x + spriteAtlasEntry.sourceSize.w)
    }px ${
      atlasDimensions.h -
      (spriteAtlasEntry.frame.y + spriteAtlasEntry.sourceSize.h)
    }px ${spriteAtlasEntry.frame.x}px)`,
  } as Record<string, string>;
