...
export function wethEnv(networkName: string): string {
  return process.env[networkName.toUpperCase() + '_WETH'];
}

export function stateTransferEnv(networkName: string): {
  ChildChainManager: string;
  _fxChild: string;
} {
  return {
    ChildChainManager:
      process.env[networkName.toUpperCase() + '_CHECKPOINT_MANAGER'],
...
export function correspondingNetworkName(network: string): string {
  switch (network) {
    case 'mumbai':
      return 'goerli';
    case 'matic':
